...
class StockEnvTrade(gym.Env):
    ...
    def __init__(self, df, day=0, initial=True):
        ...
        self.action_space = spaces.Box(
            low=-1, high=1, shape=(self.stock_dim, )
        )
        ...
        self.state = [INITIAL_ACCOUNT_BALANCE
                      ] + self.data.values.tolist() + [0] * self.stock_dim
    ...
    def _buy_stock(self, index, action):
        ...
        self.state[0] -= self.state[index +
                                    1] * action * (1+TRANSACTION_FEE_PERCENT)
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            buy_index = argsort_actions[::-1][:np.where(actions > 0)[0].
                                              shape[0]]
...
def train(algo, policy, env_train, timesteps, seed=42, save=True):
    start = time.time()
    if algo == "PPO":
        ...
        model = DQN(policy, env_train, verbose=0, seed=seed)
